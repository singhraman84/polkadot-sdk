...
pub enum MultiSignature {
	...
	Falcon512(Falcon512MultiSig),
	Falcon1024(Falcon1024MultiSig),
}
...
impl From<Falcon512MultiSig> for MultiSignature {
    fn from(x: Falcon512MultiSig) -> Self {
        MultiSignature::Falcon512(x)
    }
}

impl TryFrom<MultiSignature> for Falcon512MultiSig {
    type Error = ();

    fn try_from(m: MultiSignature) -> Result<Self, Self::Error> {
        if let MultiSignature::Falcon512(x) = m {
            Ok(x)
        } else {
            Err(())
        }
...
impl From<Falcon1024MultiSig> for MultiSignature {
    fn from(x: Falcon1024MultiSig) -> Self {
        MultiSignature::Falcon1024(x)
    }
}

impl TryFrom<MultiSignature> for Falcon1024MultiSig {
    type Error = ();

    fn try_from(m: MultiSignature) -> Result<Self, Self::Error> {
        if let MultiSignature::Falcon1024(x) = m {
            Ok(x)
        } else {
            Err(())
        }
...
pub enum MultiSigner {
	...
	Falcon512(falcon512::Public),
	Falcon1024(falcon1024::Public),
}

impl FromEntropy for MultiSigner {
	fn from_entropy(input: &mut impl codec::Input) -> Result<Self, codec::Error> {
		Ok(match input.read_byte()? % 7 {
			...
			5 => Self::Falcon512(FromEntropy::from_entropy(input)?),
			6 => Self::Falcon1024(FromEntropy::from_entropy(input)?),
			_ => Self::Ecdsa(FromEntropy::from_entropy(input)?)
		})
	}
}
...
impl AsRef<[u8]> for MultiSigner {
	fn as_ref(&self) -> &[u8] {
		match *self {
			...
			Self::Falcon512(ref who) => who.as_ref(),
			Self::Falcon1024(ref who) => who.as_ref(),
		}
	}
}
...
impl traits::IdentifyAccount for MultiSigner {
	...
	fn into_account(self) -> AccountId32 {
		match self {
			...
			Self::Falcon512(who) => blake2_256(who.as_ref()).into(),
			Self::Falcon1024(who) => blake2_256(who.as_ref()).into(),
		}
	}
}
...
impl From<falcon512::Public> for MultiSigner {
    fn from(pk: falcon512::Public) -> Self {
        MultiSigner::Falcon512(pk)
    }
}

impl TryFrom<MultiSigner> for falcon512::Public {
    type Error = ();
    fn try_from(ms: MultiSigner) -> Result<Self, Self::Error> {
        match ms {
            MultiSigner::Falcon512(pk) => Ok(pk),
            _ => Err(()),
        }
    }
}
...
impl From<falcon1024::Public> for MultiSigner {
    fn from(pk: falcon1024::Public) -> Self {
        MultiSigner::Falcon1024(pk)
    }
}

impl TryFrom<MultiSigner> for falcon1024::Public {
    type Error = ();
    fn try_from(ms: MultiSigner) -> Result<Self, Self::Error> {
        match ms {
            MultiSigner::Falcon1024(pk) => Ok(pk),
            _ => Err(()),
        }
    }
}
...
impl std::fmt::Display for MultiSigner {
	fn fmt(&self, fmt: &mut std::fmt::Formatter) -> std::fmt::Result {
		match self {
			...
			Self::Falcon512(who) => write!(fmt, "falcon512: {:?}", who),
			Self::Falcon1024(who) => write!(fmt, "falcon1024: {:?}", who),
		}
	}
}
